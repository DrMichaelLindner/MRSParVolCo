function MRSGetTissueComp_UI

% MRSGetTissueComp_UI is designed to calculate the amount of CSF and grey 
% and white matter based on a structural nifti files and SIEMENS dicom or 
% rda MRS file.
%
...
[GMFile,GMPath]=uigetfile('*.nii','Select GM image',t1Path);
[WMFile,WMPath]=uigetfile('*.nii','Select WM image',t1Path);
[CSFFile,CSFPath]=uigetfile('*.nii','Select CSF image',t1Path);
[MRSFile,MRSpath]=uigetfile({'*.rda';'*.dcm'},'Select MRS file (.rda or .dcm)');
[outpath] = uigetdir(MRSpath, 'Select output folder');

fprintf('\nGet info from dicom file');
[n1,n2,n3,p1,p2,p3,ps_r,ps_c,r,c] = get_t1_infos(t1Path, t1File);

fprintf('\nGet info from MRS ');
if strcmp(MRSFile(end-2:end),'dcm');
    fprintf('dicom file');
    a = get_dicom_info(MRSpath, MRSFile);
elseif strcmp(MRSFile(end-2:end),'rda');
    fprintf('rda file');
    a = get_rda_info(MRSpath, MRSFile);
end
...
fprintf('\n\nWrite text file');
txtfilename=[outpath, filesep, MRSFile '_mrs_coord.txt'];
...
fprintf('\nSave Mask');
nii=load_untouch_nii([t1Path t1File]);
nii.img=mask;
savename = [outpath, filesep, 'Mask_',MRSFile];
...
txt_tiss_filename=[outpath,filesep, MRSFile(1:end-4) '_tiss_comp.gm'];
...
% -------------------------------------------------------------------------
    ...
    function [n1,n2,n3,p1,p2,p3,ps_r,ps_c,r,c] = get_t1_infos(t1Path, t1File)
...
% -------------------------------------------------------------------------
    ...
    function [a] = get_dicom_info(MRSpath, filename)
        
        dcm = dicominfo([MRSpath filename]);
...
% -------------------------------------------------------------------------
    ...
    function a = get_rda_info(MRSpath, File)
        fid = fopen([MRSpath, File]);
